...
integer format."
  (let ((contents (string-trim '(#\Newline) (uiop:read-file-string file))))
...
(defun combinations (L &aux res)
  (labels ((comb (arr path)
	     (if (endp arr)
		 (when path (push path res))
		 (progn
		   (comb (cdr arr) path)
		   (comb (cdr arr) (cons (car arr) path))))))
    (comb L nil)
    res))

(defmacro doliste ((i var L &optional result) &body body)
  "Like enumerate in python. I is index, VAR is the item in L."
  `(let ((,i 0))
     (mapc #'(lambda (,var) ,@body (incf ,i))
	   ,L)
     (let ((,var nil))
       ,result)))

(defun cycle (n)
  "Return a function that cycles an int from 0 to (- n 1). If no value
is passed to inner function, advance the cycle by one. Otherwise return
the current i."
  (let ((i (1- n)))
    #'(lambda (&optional p) ; if optional p is given, then just return current i
	(if p i (setf i (mod (1+ i) n))))))
